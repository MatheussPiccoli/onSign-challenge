...
2.  Abra o arquivo `app.js` e insira seu token de acesso na variável `ACCESS_TOKEN`[cite: 33, 34].
...
Definido como "amigos de amigos" que ainda não possuem conexão direta com o usuário[cite: 81].

- **Processamento**: O algoritmo percorre a lista de amigos diretos e, para cada um, verifica sua própria lista de amigos.
- **Filtro**: São removidos da sugestão o próprio usuário logado e qualquer pessoa que já conste em sua lista de amigos diretos[cite: 87, 88].

### 2. Sugestão de Interesses

Lista de interesses presentes no círculo de amigos do usuário, mas que o usuário ainda não possui[cite: 89].

- **Processamento**: Coleta todos os interesses de todos os amigos diretos.
- **Filtro**: Remove interesses que o usuário já possui em seu perfil, garantindo que apenas novas descobertas sejam exibidas[cite: 91].

## Requisitos Técnicos Implementados

- **Resiliência (Retry Logic)**: A API possui uma taxa de erro de 10%[cite: 95]. Implementei um mecanismo de tentativa que aguarda 1 segundo entre falhas, realizando até 3 tentativas antes de exibir uma tela de erro ao usuário[cite: 96, 97].
- **Ordenação Dinâmica**: A tabela permite ordenar os usuários por nome (A-Z) e inverter a ordem através de um botão de toggle[cite: 94].
...
- **Interface**: Layout limpo e responsivo utilizando tabelas conforme sugerido na especificação do desafio[cite: 101, 103].
